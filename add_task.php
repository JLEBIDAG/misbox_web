                                    ...
                                    <form id="AddForm">
                                        ...
                                        if ($row = $resultShed->fetch_assoc()) {
                                            ...
                                            <input type="hidden" name="time_in" id="time_in"
                                                value="<?= htmlspecialchars($time_in) ?>">
                                            <input type="hidden" name="time_out" id="time_out"
                                                value="<?= htmlspecialchars($time_out) ?>">
                                            <?php
                                        } else {
                                            ...
                                            class="form-control" required>';
                                                    ...
                                                    if ($_SESSION['PK_userID'] == $_SESSION['PK_userID']) {
                                                        if ($urow['class'] != 'HD') {
                                                            ...
                                                            echo '
                                                                <div class="form-group select-wrapper">
                                                                <select name="AddSeverity" id="AddSeverity" class="form-control" required>';
                                                            echo '<option disabled selected value="">Please Select</option>';
                                                            echo '<option value="Visit">Visit</option>
                                                            <option value="On Site">On Site</option>
                                                                <option value="Remote">Remote</option>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById("AddForm").addEventListener("submit", function (event) {
                ...
                let time_out = document.getElementById('time_out').value.trim(); // Get input time
                console.log("Raw Time Out:", time_out);

                const now = new Date(); // Get current date and time

                // Function to parse 12-hour format time into 24-hour format
                ...
                // Check if time_out contains AM/PM
                let hours, minutes;
                if (time_out.includes("AM") || time_out.includes("PM")) {
                    ({ hours, minutes } = parseTime(time_out));
                } else {
                    [hours, minutes] = time_out.split(":").map(Number); // Assume it's already in 24-hour format
                }

                // Convert time_out to a Date object with today's date
                ...
                if (classes !== 'HD' && currentTime >= timeOutDate) {
                    ...
                    swal({
                        ...
                        if (isConfirm) {
                            // User clicked "Yes" - Proceed with normal submission
                            saveTicket(form, true);
                        } else {
                            // User clicked "No" - Set handle_by to null
                            saveHandleNul(form, false);
                        }
                    });
                }
                else {
                    ...
                    success: function (response) {
                       // console.log("AJAX Success:", response);
                        ...
                        swal({
                            ...
                            window.open('activity_logs.php', '_self'); // Redirect to activity_logs.php
            ...
            function saveHandleNul(form, isHandled) {
                isSubmitting = true; // Prevent multiple submissions
