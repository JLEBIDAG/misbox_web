            ...
            ORDER BY 
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $limited_text = implode(' ', array_slice(explode(' ', $row["concern"]), 0, 20));
                        $assignedBy = ($row['Username'] == $_SESSION['Username']) ? 'You' : htmlspecialchars($row["Username"]);

                        ?>
                        <tr data-id="<?= $row["PK_activityLogID"] ?>" data-ticketNo="<?= $row["ticket_no"] ?>"
                            ...
                            <td style="width:30%">
                                <?= htmlspecialchars($limited_text) ?>             <?= count(explode(' ', $row["concern"])) > 20 ? '...' : '' ?>
                            </td>
                            <td><?= $assignedBy ?></td>
                            <td>
                                ...
                                <?php if ($row['status'] != "Completed" && $row['is_request'] == NULL): ?>
                                    <a href="view_ticket.php?id=<?= $row['PK_activityLogID'] ?>" class="btn btn-info btn-xs edit-btn"
                                        style="margin:5px;">
                                        <span class="icon-pencil"></span>
                                    </a>
