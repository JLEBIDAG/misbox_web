            ...
            <div class="container">
                <div class="page-inner">
                    <div class="page-header">
                        <h3 class="fw-bold mb-3">Ticket Status</h3>
                                    ...
                                    <div class="table-responsive">
                                        <table id="basic-datatables"
                                            ...
                                            <tbody id="filteringTable">
                                                <?php
                                                $sql = "SELECT a.*, u.*, 
                                                    TIMESTAMPDIFF(DAY, a.created_at, a.dateTimeAccomp) AS days,
                                                    MOD(TIMESTAMPDIFF(HOUR, a.created_at, a.dateTimeAccomp), 24) AS hours,
                                                    MOD(TIMESTAMPDIFF(MINUTE, a.created_at, a.dateTimeAccomp), 60) AS minutes,
                                                    MOD(TIMESTAMPDIFF(SECOND, a.created_at, a.dateTimeAccomp), 60) AS seconds
                                                FROM activity_logs a
                                                INNER JOIN tbl_users u ON a.FK_userID = u.PK_userID
                                                WHERE a.FK_userID = ? 
                                                    AND a.handle_by != ? 
                                                ...
                                                ORDER BY 
                                                    CASE 
                                                        ...
                                                        WHEN a.priority = 'High' AND a.status != 'Completed' THEN 1
                                                        WHEN a.priority = 'Medium' AND a.status != 'Completed' THEN 2
                                                        WHEN a.priority = 'Low' AND a.status != 'Completed' THEN 3
                                                        ELSE 4
                                                    END";

                                                $stmt = $conn->prepare($sql);
                                                $stmt->bind_param("ss", $_SESSION['PK_userID'], $_SESSION['Username']);
                                                $stmt->execute();
                                                $result = $stmt->get_result();
                                                ...
                                                if ($result->num_rows > 0):
                                                    while ($row = $result->fetch_assoc()):
                                                        ...
                                                        <tr data-id="<?= $row["PK_activityLogID"] ?>"
                                                            ...
                                                            <td><?= htmlspecialchars($row["handle_by"]) ?></td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                ...
                                                                <a href="view_ticket_status.php?id=<?= $row['PK_activityLogID'] ?>"
